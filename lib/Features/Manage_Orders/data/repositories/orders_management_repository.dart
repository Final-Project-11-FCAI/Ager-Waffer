import 'package:ager_waffer/Base/common/config.dart';
import 'package:ager_waffer/Base/common/shared_preference_manger.dart';
import 'package:ager_waffer/Base/network/network_util.dart';
import 'package:ager_waffer/Features/Manage_Orders/data/models/orders_management_model.dart';
import 'package:localize_and_translate/localize_and_translate.dart';

class OrdersManagementRepository {

  Future<OrdersManagementModel> getOrdersManagement() async {
    return NetworkUtil.internal().get(
      OrdersManagementModel(),
      baseUrl + ordersManagementUrl,
      headers: Map<String, String>.from({
        'Accept': 'application/json',
        "Authorization":
        "Bearer ${await sharedPreferenceManager.readString(CachingKey.AUTH_TOKEN)}",
        'Language': LocalizeAndTranslate.getLanguageCode().toUpperCase(),
      }),
    );
  }
}

OrdersManagementRepository ordersManagementRepository = OrdersManagementRepository();