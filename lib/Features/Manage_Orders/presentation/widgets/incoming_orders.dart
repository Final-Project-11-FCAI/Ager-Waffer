...
class _IncomingOrdersState extends State<IncomingOrders> {
  ...
  Widget build(BuildContext context) {
    return BlocListener<AcceptOrderBloc, AcceptOrderState>(
      ...
      child: BlocListener<DeclineOrderBloc, DeclineOrderState>(
        listener: (context, state) {
          if (state.status == declineOrderStatus.loading) {
            Shared.showLoadingDialog(context: context);
          } else if (state.status == declineOrderStatus.success) {
            Shared.dismissDialog(context: context);
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                backgroundColor: kGreenColor,
        ...
        child: BlocBuilder<OrdersManagementBloc, OrdersManagementState>(
          builder: (context, state) {
            if (state.status == ordersManagementStatus.loading) {
              ...
              final incomingOrders = ordersManagement
